---
id: minsnaptrajectory
title: Minimum Snap Trajectory
sidebar_label: Minimum Snap Trajectory
---

## Overview

The minimum-snap trajectory is a 7th order polynomial. The whole trajectory is divided into a series of segments indicated by waypoints. Each of these segments has a polynomial-based trajectory from its starting waypoint to an ending waypoint. This relationship can be described by the Nth-order polynomial (P(t)) as shown below. However, this function only covers the trajectory of one axis. Thus, in order to fully represent a 3D trajectory, the formula must be used on all axes.

1. $P(t) = a_0 + a_1t + a_2t^2 + a_3t^3 + ... + a_Nt^N$
2. $p = [ a_0 , a_1 , a_2, ··· , a_N]^T$, where $p_{size}(N * 1)$ are polynomial coefficients
3. 
